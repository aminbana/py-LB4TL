...
class And(formula.Formula):
    ...
    def abstract_str(self):
        t = self.t0
        st = ' and '.join([f.at(t).abstract_str() for f in self.formulas])
        return f'({st})'
    ...
    def at(self, t:int):
        t = t + self.t0
        return And(self.args, self.formulas, t0 = t)
    ...
    def fill_neural_net(self, net, expected_layer_to_output:int):
        ...
        net[layer]['W1'][(idx_1, idx_middle+1)] = 1

        net[layer]['gates'].append('Softplus')
